...
def connect_muse():
    ...
    while time.time() - start_time <= config.STREAM_TIME:
        ...
        # calculate features and forwards them
        
        data, ts = stream.get_data(winsize=None, picks=['TP9', 'AF7', 'AF8', 'TP10'])
        data = data*1e-6 #convert to mV
        # remove mean
        ...
        print(scores)
        # Collect scores every 10 seconds to json file
...
@app.route('/')
def get_info():
    response = str(avg_scores[0]) + '\n' + str(avg_scores[1])
    print(response)
    return response

@app.before_request
def start_muse_stream():
    app.before_request_funcs[None].remove(start_muse_stream)

    thread = threading.Thread(target=connect_muse)
    thread.daemon = True
    thread.start()

serve(app, listen='*:1234')
